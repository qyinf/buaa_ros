...
from team_3.srv import Base, BaseResponse
    ...
    def init(self, req):
        ...
        def t():
            if rospy.get_param("simulate"):
                os.system("roslaunch team_3 sim_navigation.launch")
            else:
                os.system("roslaunch team_3 robot_navigation.launch")
                
        p = multiprocessing.Process(target=t)
        p.start()
        self.pid = p.pid
        s = Waypoints().getNames()
        return BaseResponse("开始校准，已有航点" + str(len(s)) + "个： " + ' '.join(s))
